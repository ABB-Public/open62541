...
class CGenerator:
    ...
    def print_datatype(self, datatype, namespaceMap):
        typeid = "{{{}, {}}}".format(namespaceMap[datatype.namespaceUri], getNodeidTypeAndId(datatype.nodeId))
        binaryEncodingId = "{{{}, {}}}".format(namespaceMap[datatype.namespaceUri], getNodeidTypeAndId(datatype.binaryEncodingId))
        xmlEncodingId = "{{{}, {}}}".format(namespaceMap[datatype.namespaceUri], getNodeidTypeAndId(datatype.xmlEncodingId))
        ...
        if totalCount > 0:
            ...
            for ns in self.filtered_types:
                for i, t_name in enumerate(self.filtered_types[ns]):
                    ...
                    self.printh("#define UA_" + makeCIdentifier(self.parser.outname.upper() + "_" + t.name.upper()) + " " + str(idx))
                    idx += 1
                    self.printh("")
                    if not is_cross_ns_dup:
