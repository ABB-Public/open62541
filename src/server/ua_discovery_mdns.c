...
addMdnsRecordForNetworkLayer(UA_DiscoveryManager *dm, const UA_String serverName,
    ...
    if(hostname.length == 0 && !UA_String_isEmpty(&serverName)) {
        hostname = serverName;
    }
...
discovery_createMulticastSocket(UA_DiscoveryManager *dm) {
    ...
    if(dm->sc.server->config.mdnsInterfaceIP.length > 0) {
        ...
        UA_Variant_setScalar(&params[paramsSize].value, &dm->sc.server->config.mdnsInterfaceIP,
                             &UA_TYPES[UA_TYPES_STRING]);
        paramsSize++;
    }
    if(dm->sc.server->config.mdnsIpAddressListSize > 0) {
        params[paramsSize].key = UA_QUALIFIEDNAME(0, "multicastAddrList");
        UA_Variant_setArray(&params[paramsSize].value, dm->sc.server->config.mdnsIpAddressList,
                            dm->sc.server->config.mdnsIpAddressListSize, &UA_TYPES[UA_TYPES_UINT32]);
        paramsSize++;
    }
